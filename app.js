...
if (formFinance) {
    formFinance.addEventListener('submit', (e) => {
        ...
        const type = document.getElementById('fin-type').value; // IN or OUT
        ...
        const newData = {
            id: generateID(),
            type: 'FINANCE',
            finType: type, // Sub-type for finance
            date,
            category,
            customer: category, // Using category as 'entity' name for list display
            total: amount,
            pay: amount, // Finance entries are always fully 'paid' in this context
            debt: 0,
            note,
            status: 'LUNAS',
            timestamp: new Date().getTime()
        ...
        refreshCurrentView();
        showToast('Tersimpan', `Transaksi ${category} berhasil dicatat`, 'success');
        formFinance.reset();

        // Reset date to current for next entry
        const nowISO = new Date(new Date().getTime() - (new Date().getTimezoneOffset() * 60000)).toISOString().slice(0, 16);
...
function renderKeuangan() {
    ...
    const uangMasukManual = finances.filter(f => f.finType === 'IN').reduce((sum, f) => sum + f.total, 0);
    const uangKeluarManual = finances.filter(f => f.finType === 'OUT').reduce((sum, f) => sum + f.total, 0);
    ...
    const filtered = sorted.filter(t => {
        ...
        return category.includes(searchTerm) || itemsDetail.includes(searchTerm) || typeLabel.includes(searchTerm);
    });
    ...
    <div class="transaction-card">
        <div class="t-left">
            <h4 style="display: flex; align-items: center; gap: 8px;">
                <span class="status-pill ${isIn ? 'status-paid' : 'status-unpaid'}" style="padding: 2px 8px; font-size: 0.75rem;">
                    ${isIn ? 'Masuk' : 'Keluar'}
                </span>
                ${title}
            </h4>
...
function renderDashboard() {
    ...
    const cashInManual = finances.filter(f => f.finType === 'IN').reduce((sum, f) => sum + f.total, 0);
    const cashOutManual = finances.filter(f => f.finType === 'OUT').reduce((sum, f) => sum + f.total, 0);
